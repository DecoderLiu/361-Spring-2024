...
- Midterms, both during our class time and in our classroom: 
    - Tue Feb 20, 9am-10:15am
    - Tue Apr 2, 9am-10:15am
...
Please also install/update the packages listed in the first assignment. We will use those throughout the course. We may end up needing other packages too, which we'll install on the fly. But we will definitely use the packages at that link, so take the time to download them now. 
...
**Exam format.** Exams will have two parts. Part 1 will be pencil and paper only. There will be some conceptual questions, questions where I give output (e.g. `summary(lm1)` output, a data visualization) that you have to analyze, questions where I give you code that you have to comment, and questions where you have to write code. 

Part 2 will be open notes, open computer, open internet. You can not communicate with another human, but you can communication with an AI chatbot (e.g. ChatGPT, Claude, Bard/Gemini, Grok, etc). You will have to write code to answer questions.  Often you have perform some sort of data exploration and visualization, fit a model, and display and analyze the results.

Practice problems will be given a week before the exam.  

**Makeup exams**. With a Dean's note, you can take a makeup exam. We will schedule the makeup within a few days of the original exam based on students' availability. 
...
**Ed Discussion and GitHub Issues and Pull Requests.** You will be able to ask questions on Ed Discussion and GitHub issues.  The same guidelines as office hours apply here: You are expected to attempt problems before seeking help, and when you ask for help, clearly state what you have attempted, where you got stuck, and what specific question you have. Use minimal reproducible examples when possible. 

If you notice a typo in an assignment, you can create a pull-request with the correction. If you need clarification on an assignment, instead of asking on Gradescope, you may also  ask a question using an issue. Note that you can refer to a file and line number within the issue by doing this: 

- Go to the file, and hover over the line. 
- Click the 3 dots on the left
