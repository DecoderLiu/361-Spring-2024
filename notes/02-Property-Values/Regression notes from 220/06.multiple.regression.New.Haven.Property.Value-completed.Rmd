...
```{r}
ggpairs(d[,c('living', 'land', 'beds', 'baths')])
```
```{r}
cor(d[,c('living', 'land', 'beds', 'baths')], 
...
The adjusted $R^2$ for `m4` is a little higher than `m2`, and this time the coefficient for `baths` has a positive sign. The coefficients for `living` and `land` changed a little, but not as much as they did when including `beds`. So we probably want to include `baths` because 
