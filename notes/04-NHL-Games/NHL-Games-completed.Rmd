...
```{r}
n.frames = length(seq(2.0, 3.75, by=0.1))

## lines
dl = data.frame(left  = seq(2.0, 3.5, length=n.frames), 
                right = seq(2.5, 4.00, length=n.frames), 
                frame = 1:n.frames) %>%
...
dh = dh %>%
  mutate(jitter.score = jitter(score, 
                               amount = 0.2))
...
ddh2 = ddh %>% 
  mutate(hist.score = ifelse(inside == TRUE, 
                             score, 
                             NA),
         avg.score.prev = NA,
...
g = ggplot(ddh %>% filter(frame %in% c(1:n.frames)), ## uncomment to test w/ 2 frames
       aes(x = avg.score.prev, 
           y = jitter.score, 
           color = inside, 
           group = gid))+
  ...
  scale_color_manual(values = c(publightgray,
                                pubblue))+
  geom_vline(data = dl %>% filter(frame %in% c(1:n.frames)), 
             aes(xintercept = left), 
             color = pubdarkgray)+
  geom_vline(data = dl %>% filter(frame %in% c(1:n.frames)),
             aes(xintercept = right), 
             color = pubdarkgray)+
  geom_point(data = dl %>% filter(frame %in% c(1:n.frames)) %>%
             ...
             size = 2,
             inherit.aes = F)+
  geom_segment(data = dl %>% filter(frame %in% c(1:n.frames)),
               aes(x    = mid , 
                   xend = end,
                   y    = mean, 
                   yend = mean),
...
# a = animate(gg,
#             fps     = 10,
#             nframes = 2*n.frames + 14,
#             height  = 320,
#             width   = 480,
#             end_pause = 14)
# a
# 
# anim_save(a, filename = 'img/dist.of.score.for.different.subsets.nhl.gif')

```

![](img/dist.of.score.for.different.subsets.nhl.gif)
...
```{r}
coefs = rbind(data.frame(summary( lm1)$coefficients[,1:2], model='Linear'),
              data.frame(summary(glm1)$coefficients[,1:2], model='Poisson')) %>%
  mutate(varname = rep(c('(Intercept)', 'avg.score.prev'),2)) %>%
  rename(est = Estimate,
         se  = Std..Error) %>%
  arrange(varname) %>%
  remove_rownames() %>%
  select(varname, model, est, se) 
coefs
```
...
In this case, the lines are pretty close, though the Poisson regression curve is more, well, curved. And again, it's not all about the prediction of the mean value in this example. They are pretty close. It's about the uncertainties, and the distribution of `score` for a given value of the predictor. 
...
confint(glm1)
exp(confint(glm1))
summary(glm1)$coefficients[2,1] - qnorm(.975, 0, 1)*summary(glm1)$coefficients[2,2]
summary(glm1)$coefficients[2,1] + qnorm(.975, 0, 1)*summary(glm1)$coefficients[2,2]
...
**find pred and se.fit for 'link' then exp(), or find pred and se.fit for 'response'? Can skip this for now.**
...
Let's verify the interpretation of the coefficients for the Poisson regression model. 


```{r}
df = data.frame(avg.score.prev = seq(0, 5.0, by=1))

df = df %>%
  mutate(pred  = predict(glm1, newdata=df, type='response'),
         ratio = pred/lag(pred),
         link  = predict(glm1, newdata=df, type='link'), 
         diff  = link - lag(link))
df

df$ratio
df$diff 
glm1$coefficients['avg.score.prev'] %>% exp()
glm1$coefficients['avg.score.prev']
```
...
```{r}
dh = dh %>% filter(season == 2022)
lm2  =  lm(score ~ ha + team + opp, data=dh)
glm2 = glm(score ~ ha + team + opp, data=dh, family='poisson')
