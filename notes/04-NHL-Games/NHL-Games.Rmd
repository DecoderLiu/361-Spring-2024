...
```{r}
dh$glm1.pred = predict(glm1, newdata=dh, type='response')
...
```{r}
lm2 = lm(score ~  ha + team + opp, 
         data = dh, filter(season == 2022)) 
summary(lm2)
glm2 = glm(score ~  ha + team + opp, 
           data = dh, 
           family = 'poisson', filter(season == 2022))
summary(glm2)
