...
boot.results = data.frame(j=1:J, coef=NA, log.lambdahat=NA) 
...
for(j in 1:J){
  ...
  boot.results[j, 'log.lambdahat'] = predict(m1, newdata=dp, type='link') 
...
dc %>% 
  ...
  group_by(name) %>%
  summarise(rmse = sqrt(mean(err^2, na.rm = T)))
```
